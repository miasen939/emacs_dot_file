...
** 番茄钟
番茄/计时：org-pomodoro（和 org-agenda 集成最好），或 pomm（transient 菜单超美）、tmr（Protesilaos 出品，轻量定时器）
** 智能 C-a M-a M-e C-e 智能 C-backspace删除
** popper
...
** 撤出minimal emacs 真正的香草味

** 如何更新transit/orgmode这些内置包？
把更新内置包的变量设为t，直接用 M-x package-update 就行了
** stright.el

** rime调试配置

** vertico + consult + orderless + marginalia + embark现代 Emacs 的“灵魂四件套”，搜索/跳转/操作快到飞起
...
👉 这一块其实决定你“住得舒不舒服”，比功能更重要。

🔍 搜索 / 导航（可以强化）

你写了 consult，但可以补完整生态：
